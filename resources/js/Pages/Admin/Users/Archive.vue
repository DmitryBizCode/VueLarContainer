...
const applyFilters = () => filterForm.get(route('admin.users.archive'), { preserveState: true });

const formatDate = (v) =>
    v ? new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(v)) : '—';
const DEFAULT_PROFILE_IMAGE = "data:image/svg+xml," + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="%2394a3b8" stroke-width="1.5"><circle cx="12" cy="8" r="3"/><path d="M5 20v-2a5 5 0 0110 0v2"/></svg>');
...
const restoreUser = (id) => {
    router.post(route('admin.users.restore', id), {}, { preserveScroll: true });
};
...
const confirmForceDelete = () => {
    ...
    forceDeleteForm.post(route('admin.users.force-destroy', forceDeleteTarget.value.id), {
        preserveScroll: true,
...
<template>
    <Head title="Admin – User archive" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h1 class="text-xl font-bold text-slate-900">Admin – User archive</h1>
                <Link
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <p class="mb-4 text-sm text-slate-600">Users that were deleted. You can restore them or remove permanently.</p>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        ...
                        <tbody class="divide-y divide-slate-200">
                            <tr v-for="u in users.data" :key="u.id" class="bg-white hover:bg-slate-50/50">
                                <td class="px-4 py-2">
                                    <img :src="profilePhotoUrl(u.photo)" :alt="u.email" class="h-10 w-10 rounded-full object-cover" />
                                </td>
                                ...
                                <td class="px-4 py-2 text-sm text-slate-600">{{ formatDate(u.deleted_at) }}</td>
                ...
                <div v-if="!users.data?.length" class="rounded-xl border border-slate-200 bg-white py-12 text-center text-slate-500">
                    No archived users.
                </div>
        ...
        <Modal :show="forceDeleteModalOpen" @close="closeForceDeleteModal">
            <div class="rounded-xl bg-white p-6 shadow-lg">
                <h3 class="text-lg font-bold text-slate-900">Permanently delete user?</h3>
                <p class="mt-2 text-sm text-slate-600">
                    User <strong>{{ forceDeleteTarget?.email }}</strong> will be removed forever. This cannot be undone.
                </p>
