...
const props = defineProps({ routeData: { type: Object, required: true }, ports: { type: Array, default: () => [] } });
const form = useForm({
    origin_port_id: props.routeData.origin_port_id,
    destination_port_id: props.routeData.destination_port_id,
    estimated_days: props.routeData.estimated_days,
    distance: props.routeData.distance,
    route_status: props.routeData.route_status,
});
const submit = () => form.put(route('admin.routes.update', props.routeData.id));
</script>

<template>
    <Head title="Admin – Edit route" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h1 class="text-xl font-bold text-slate-900">Edit route #{{ routeData.id }}</h1>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-md px-4 sm:px-6 lg:px-8">
                <form class="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submit">
                    ...
                    <div class="flex gap-2"><button type="submit" class="rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700" :disabled="form.processing">Update</button><Link :href="route('admin.routes.index')" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700">Cancel</Link></div>
                </form>
            </div>
        </div>
    </AuthenticatedLayout>
