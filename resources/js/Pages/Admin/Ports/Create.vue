...
const props = defineProps({ countries: { type: Array, default: () => [] } });
const form = useForm({ country_id: '', name: '', city: '' });
const submit = () => form.post(route('admin.ports.store'));
</script>

<template>
    <Head title="Admin – Add port" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h1 class="text-xl font-bold text-slate-900">Add port</h1>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-md px-4 sm:px-6 lg:px-8">
                <form class="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submit">
                    ...
                    <div class="flex gap-2"><button type="submit" class="rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700" :disabled="form.processing">Create</button><Link :href="route('admin.ports.index')" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700">Cancel</Link></div>
                </form>
            </div>
        </div>
    </AuthenticatedLayout>
